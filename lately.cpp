...
Lately::Lately(QWidget *parent)
    : QWidget{parent}
{
    manager=new QNetworkAccessManager(this);
    connect(manager,&QNetworkAccessManager::finished,this,&Lately::noReplied);

    connect(this,&Lately::nextitem,this,&Lately::do_nextitem);

    this->setStyleSheet("background-color: white; border: none;");
    label_1=new QLabel(this);
    label_1->setText("最近播放");
    ...
    connect(listwidget,&QListWidget::itemClicked,this,&Lately::do_itemClicked);


    loveidlist=LinkQSL::heardlist;
...
void Lately::do_itemClicked(QListWidgetItem *item)
{
    ...
    emit openmusicwidget(2);
}

void Lately::noReplied(QNetworkReply *reply)
{
    ...
    //初始化每个歌曲行
    if(LinkQSL::lovemusic[loveidlist[lovenamelist.count()-1]]){
        ...
        itemmusic->setinit(loveidlist[lovenamelist.count()-1],lovenamelist[lovenamelist.count()-1],loveauthorlist[loveauthorlist.count()-1],lovealbumlist[lovealbumlist.count()-1],lovefeelist[lovefeelist.count()-1],0);
    }
